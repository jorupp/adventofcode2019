    ...
    public class Part1 : BasePart
    {
        protected void RunScenario(string title, string input)
        {
            RunScenario(title, () =>
            {
                ...
                var data = lines.Select(i => i.Numbers()).Select(i => new Point{ x = i[0], y = i[1], dx = i[2], dy=i[3] }).ToArray();
                ...
                for(var t = 1; true; t++)
                {
                    ...
                    // how do I know if to stop...
                    var xMin = data.Min(i => i.x);
                    var xMax = data.Max(i => i.x);
                    var yMin = data.Min(i => i.y);
                    var yMax = data.Max(i => i.y);
                    if (Math.Abs(yMax - yMin) < 40)
                    {
                        ...
                        for (var y = yMin; y <= yMax; y++)
                        {
                            ...
                            {
                                sb.Append(points.Contains((x, y)) ? 'X' : '.');
                            }
        ...
        {
//            RunScenario("initial", @"position=< 9,  1> velocity=< 0,  2>
...
//");
            //return;
            RunScenario("part1", @"position=<-31761,  10798> velocity=< 3, -1>
