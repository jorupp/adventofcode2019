            ...
            RunScenario(title, () =>
            {
                ...
                for (var t = 0; ; t++)
                {
                    ...
                    foreach (var cart in toProcess)
                    {
                        ...
                        var otherCart = carts.FirstOrDefault(i => i.X == cart.X && i.Y == cart.Y && i != cart && !i.IsCrashed);
                        if (null != otherCart)
                        {
                            Console.WriteLine($"CRASH {cart.X}, {cart.Y} => {t}");
                            cart.IsCrashed = true;
                            otherCart.IsCrashed = true;
                        }
                    }

                    if (carts.Count(i => !i.IsCrashed) == 1)
                    {
                        var cart = carts.Single(i => !i.IsCrashed);
