...
namespace AoC.Year2018.Day24
{
    ...
    {
        protected void RunScenario(string title, Team[] inTeams)
        {
            RunScenario(title, () =>
            {
                for (var i = 0;; i++)
                {
                    var newTeams = inTeams.Select(ii => new Team(ii)).ToArray();
                    foreach (var g in newTeams[0].Groups)
                    {
                        g.Damage += i;
                    }
                    var result = Simulate(newTeams);
                    Console.WriteLine($"Checking {i} = {result}");
                    if(result > 0)
                        return;
                }

                int Simulate(Team[] teams)
                {
                    ...
                    while (true)
                    {
                        ...
                        {
                            var attackingGroups = attacker.Groups.Where(i => i.IsAlive)
                                .OrderByDescending(i => i.EffectivePower)
                            ...
                            foreach (var ag in attackingGroups)
                            {
                                var defenders = defender.Groups.Where(i => i.IsAlive && !targets.Values.Contains(i))
                                    ...
                                    .OrderByDescending(i => i.dmg).ThenByDescending(i => i.i.EffectivePower)
                                    .ThenByDescending(i => i.i.Initiative)
                                    .ToArray();
                        ...
                        if (killsThisRound == 0)
                        {
                            // we can get into a situation where there are units alive on each side, but none are strong enough to kill a unit on the other side
                            // since we're looking for a clear winner, we'll call this a draw and move on
                            return 0;
                        }
                    }

                    var margin = teams.Sum(t => t.Groups.Where(g => g.IsAlive).Sum(g => g.Units));
                    if (teams[0].Groups.Any(i => i.IsAlive))
                    {
                        return margin;
                    }
                    else
        ...
        public class Team
        {
            ...
            public Team(Team old)
            {
                Name = old.Name;
                Groups = old.Groups.Select(i => new Group(i)).ToArray();
            }
        }
