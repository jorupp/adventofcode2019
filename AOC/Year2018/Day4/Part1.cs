    ...
    public class Part1 : BasePart
    {
        protected void RunScenario(string title, string input)
        {
            RunScenario(title, () =>
            {
                ...
                var records = lines.Select(i =>
                {
                    var parts = regEx.Match(i).Groups.Skip(1).Select(x => x.Value).ToList();
                    return new
                    {
                        time = new DateTimeOffset(int.Parse(parts[0]) + 500, int.Parse(parts[1]), int.Parse(parts[2]), int.Parse(parts[3]), int.Parse(parts[4]), 0, TimeSpan.Zero),
                        wakeup = parts[5] == "wakes up",
                        asleep = parts[5] == "falls asleep",
                        guardNum = parts[5].Contains("Guard") ? parts[5].Numbers().Single() : (int?) null,
                    };
                }).OrderBy(i => i.time).ToList();

                var asleepFreq = new Dictionary<int, int[]>();
                var asleepMinutes = new Dictionary<int, int>();
                var guard = 0;
                DateTimeOffset lastTime = DateTimeOffset.MinValue;
                foreach (var rec in records)
                {
                    if (rec.guardNum.HasValue)
                    {
                        guard = rec.guardNum.Value;
                        goto end;
                    }
                    if (rec.wakeup)
                    {
                        int[] asleep;
                        if (!asleepFreq.TryGetValue(guard, out asleep))
                        {
                            asleep = new int[60];
                            asleepFreq.Add(guard, asleep);
                        }

                        for (var i = lastTime.Minute; i < rec.time.Minute; i++)
                        {
                            asleep[i]++;
                        }

                        asleepMinutes[guard] = (asleepMinutes.TryGetValue(guard, out var value) ? value : 0) + (int)rec.time.Subtract(lastTime).TotalMinutes;
                    }
                ...
                var sleepyGuardId = asleepMinutes.OrderByDescending(i => i.Value).First().Key;
                var bestMinute = asleepFreq[sleepyGuardId].Select((freq, minute) => new {freq, minute }).OrderByDescending(i => i.freq).First().minute;

                Console.WriteLine($"sleepy: {sleepyGuardId}");
                Console.WriteLine($"bestMinute: {bestMinute}");
                Console.WriteLine($"answer: {sleepyGuardId * bestMinute}");
            });
        }

        public override void Run()
        {
            RunScenario("initial", @"[1518-11-01 00:00] Guard #10 begins shift
...
[1518-11-05 00:55] wakes up");
            //return;
            RunScenario("part1", @"[1518-05-11 00:47] wakes up
