    ...
    public class Part2 : BasePart
    {
        protected void RunScenario(string title, string input)
        {
            RunScenario(title, () =>
            {
                ...
                var sleepy = asleepFreq.SelectMany(guardFreq => guardFreq.Value.Select((freq, minute) => new { guardId = guardFreq.Key, freq, minute })).OrderByDescending(i => i.freq).First();

                Console.WriteLine($"sleepy: {sleepy.guardId}");
                Console.WriteLine($"bestMinute: {sleepy.minute}");
                Console.WriteLine($"answer: {sleepy.guardId * sleepy.minute}");
            });
        }
