    ...
    public class Part1a : BasePart
    {
        protected void RunScenario(string title, string input)
        {
            RunScenario(title, () =>
            {
                ...
                var maxRuns = GetMaxRuns(reactions, "FUEL", 1);
                var maxOutputs = (from run in maxRuns
                        join r in reactions on run.Key equals r.Key
                        select new {key = r.Key, q = run.Value * r.Value}
                        ).ToList();
                var maxInputs = (from run in maxRuns
                        join r in reactions on run.Key equals r.Key
                        from i in r.Item3
                        select new { key = i.Key, q = -run.Value * i.Value }
                    ).ToList();

                var maxQ = maxOutputs.Concat(maxInputs).GroupBy(i => i.key)
                    .ToDictionary(i => i.Key, i => i.Sum(ii => ii.q));

                Console.WriteLine(string.Join(",", maxRuns.OrderBy(i => i.Key).Select(i => $"{i.Key}={i.Value}")));

                var start = new ChemNode(maxQ, reactions);//, new Estimator(reactions), new Dictionary<string, long>());
                //var solution = new RealSolver().Evaluate<ChemNode, string, long>(start.GetAdjacent().Append(start).ToArray());
                new RealSolver().Evaluate<ChemNode, string, long>(new[] { start }, null, null, null, (bestNodes, x, y) =>
                {
                    var solution = bestNodes.Values.Where(i => i.IsValid).OrderBy(i => i.CurrentCost).First();

                    Console.WriteLine($"{solution.Description} - {solution.Ore}");
                });

                //Console.WriteLine(solution.Description);
                //Console.WriteLine(solution.Ore);


            });
        }

        private Dictionary<string, long> GetMaxRuns(List<(string, long, Dictionary<string, long>)> reactions, string key, long quantity)
        {
            if (key == "ORE")
            {
                return new Dictionary<string, long>() { { key, quantity}};
            }

            var reaction = reactions.Single(i => i.Item1 == key);
            var q = (long)Math.Ceiling((decimal)quantity / reaction.Item2);
            var subReactions = 
                reaction.Item3.Select(i => GetMaxRuns(reactions, i.Key, i.Value * q))
                .ToList();
            ...
            foreach (var sub in subReactions)
            {
                foreach (var kvp in sub)
                {
                    output[kvp.Key] = (output.TryGetValue(kvp.Key, out var v) ? v : 0) + kvp.Value;
                }
            }
        ...
        private class ChemNode : Node<ChemNode, string, long>
        {
            ...
            public override IEnumerable<ChemNode> GetAdjacent()
            {
                ...
                foreach (var reaction in _reactions)
                {
                    ...
                    node._stuff[reaction.Item1] = oldOutputValue - reaction.Item2;

                    foreach (var input in reaction.Item3)
                    {
                        node._stuff[input.Key] = (node._stuff.TryGetValue(input.Key, out var v) ? v : 0) + input.Value;
                    }
