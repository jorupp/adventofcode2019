...
namespace AoC.Year2019.Day15
{
    public class Part1a : BasePart
    {
        ...
        private class MoveNode : Node<MoveNode, (long, long), long>
        {
            ...
            private readonly IntCodeSimulationState _simState;

            public MoveNode(long x, long y, long cost, Dictionary<(long, long), int> map, IntCodeSimulationState simState)
            {
                _x = x;
                _y = y;
                _cost = cost;
            ...
            public override IEnumerable<MoveNode> GetAdjacent()
            {
                ...
                {
                    var nextState = _simState.Resume(dir.Key);
                    var nextResult = nextState.Output.Single();
                    if (nextState.Halted)
                    {
                        throw new NotImplementedException();
                    }
                    var nextX = _x + dir.Value.Item1;
                    var nextY = _y + dir.Value.Item2;
                    _map[(nextX, nextY)] = (int)nextResult;

                    if (nextResult == 0)
                        continue;
                    
                    yield return new MoveNode(nextX, nextY, _cost + 1, _map, nextState);
                }
            }
        ...
        protected void RunScenario(string title, string input)
        {
            RunScenario(title, () =>
            {
                ...
                var start = new MoveNode(0,0, 0, map, IntCodeSimulationState.Start(data));
                ...
                void Dump()
                {
                    ...
                    for (var y = nmx; y <= my; y++)
                    {
                        ...
                        {
                            var value = (map.TryGetValue((x, y), out var v) ? v : -1);
                            Console.Write(y == 0 && x == 0 ? "S" : value == 0 ? "#" : value == 1 ? "." : value == 2 ? "T" : " ");
                        }
                        Console.WriteLine();
                    }
                    Console.WriteLine();
