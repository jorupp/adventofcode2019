    ...
    public class Part2 : BasePart
    {
        protected void SolveIt(string title, string input)
        {
            RunScenario(title, () =>
            {
                ...
                var start = new MoveNode(targetP.Item1, targetP.Item2, 0, map);
                var noSolution = new RealSolver().Evaluate<MoveNode, (long, long), long>(new [] { start }, null, null, null,
                    (bestNodes, todo, done) =>
                    {
                        Console.WriteLine(bestNodes.Max(i => i.Value.CurrentCost));
                    });
        ...
        private class MoveNode : Node<MoveNode, (long, long), long>
        {
            ...
            public MoveNode(long x, long y, long cost, Dictionary<(long, long), int> map)
            {
                _x = x;
                _y = y;
                _cost = cost;
            ...
            {
                yield return new MoveNode(_x, _y + 1, _cost + 1, _map);
                yield return new MoveNode(_x, _y - 1, _cost + 1, _map);
                yield return new MoveNode(_x + 1, _y, _cost + 1, _map);
                yield return new MoveNode(_x - 1, _y, _cost + 1, _map);
            }
