...
".Replace("\r", "").Select(i => (int)i).ToList();
        ...
        protected void RunScenario(string title, string input)
        {
            RunScenario(title, () =>
            {
                ...
                var start = map.Single(i => i.Value == '^').Key;
                var dir = 0;

                string Go((int, int) start, int dir)
                {
                    var x = start.Item1;
                    var y = start.Item2;
                    ...
                    while (true)
                    {
                        var valuea = (map.TryGetValue((x + dirs[dir].Item1, y + dirs[dir].Item2), out var v) ? v : '.');
                        if(valuea == '#')
                        {
                            ahead++;
                            x += dirs[dir].Item1;
                    ...
                    if (ahead > 0)
                    {
                        return $"{ahead}," + Go((x, y), dir);
                    }

                    var ldir = (dir - 1 + 4) % 4;
                    var rdir = (dir + 1) % 4;

                    var valuel = (map.TryGetValue((x + dirs[ldir].Item1, y + dirs[ldir].Item2), out var v1) ? v1 : '.');
                    var valuer = (map.TryGetValue((x + dirs[rdir].Item1, y + dirs[rdir].Item2), out var v2) ? v2 : '.');

                    if (valuel == '#')
                    {
                        return "L," + Go(start, ldir);
                    }
                    else if(valuer == '#')
                    {
                        return "R," + Go(start, rdir);
                    }
                    else
                    {
                        return "DONE";
                ...
                Dump();
                Console.WriteLine(Go(start, dir));
            });
        }
