...
namespace AoC.Year2019.Day25
{
    public class Part1 : BasePart
    {
        ...
        static Dictionary<string, Regex> dirRes = new Dictionary<string, Regex>()
        {
            { "north", new Regex("^- north$", RegexOptions.Multiline) },
            { "south", new Regex("^- south$", RegexOptions.Multiline) },
            { "east", new Regex("^- east$", RegexOptions.Multiline) },
            { "west", new Regex("^- west$", RegexOptions.Multiline) },
        };
        ...
        protected void RunScenario(string title, string input)
        {
            RunScenario(title, () =>
            {
                ...
                void v1()
                {
                    void findstuff(IntCodeSimulationState start)
                    {
                        ...
                        var dirRes = new Dictionary<string, Regex>()
                        {
                            {"north", new Regex("^- north$", RegexOptions.Multiline)},
                            {"south", new Regex("^- south$", RegexOptions.Multiline)},
                            {"east", new Regex("^- east$", RegexOptions.Multiline)},
                        ...
                        var security = new Regex("In the next room, a pressure-sensitive floor will verify your identity.");
                        var itemsHereRe = new Regex(@"Items here:\n(.*)\n{2}");
                        var itemsRe = new Regex("^- (.*)$", RegexOptions.Multiline);

                        var doors = new Dictionary<(int, int), string[]>().WithDefault(new string[0]);
                        var items = new Dictionary<(int, int), string[]>().WithDefault(new string[0]);
                        var securityLoc = (0, 0);

                        var toCheck = new Queue<(int, int, IntCodeSimulationState)>();
                        toCheck.Enqueue((0, 0, start));
                        while (toCheck.Count > 0)
                        {
                            var item = toCheck.Dequeue();
                            var loc = (item.Item1, item.Item2);
                            ...
                            Console.WriteLine($"Current location: {loc.Item1},{loc.Item2}");
                            var output = new string(state.Output.Select(i => (char) i).ToArray());
                            Console.WriteLine(output);

                            var hasDoors = dirRes.Where(i => i.Value.IsMatch(output)).Select(i => i.Key).ToArray();
                            Console.WriteLine("Has doors: " + string.Join(", ", hasDoors));
                            doors[loc] = hasDoors;

                            var itemsHere = itemsHereRe.Match(output);
                            if (itemsHere.Success)
                            {
                                var thisItems = itemsRe.Matches(itemsHere.Groups[1].Value).Select(i => i.Groups[1].Value)
                                    .ToArray();
                                items[loc] = thisItems;
                            }

                            if (security.IsMatch(output))
                            {
                                securityLoc = loc;
                                continue;
                            }
                            ...
                            foreach (var door in hasDoors)
                            {
                                var newState = state;
                                var x = loc.Item1;
                                var y = loc.Item2;
                                var d = directions[door];
                                x += d.Item1;
                                y += d.Item2;
                                foreach (var c in door)
                                {
                                    newState = newState.Resume(c);
                                }

                                newState = newState.Resume('\n');

                                if (!doors.ContainsKey((x, y)))
                                {
                                    toCheck.Enqueue((x, y, newState));
                                }
                            }
                        }

                        var rooms = doors.Keys.OrderBy(i => i.Item1).ThenBy(i => i.Item2).ToList();
                        foreach (var room in rooms)
                        {
                            Console.WriteLine($"{room.Item1},{room.Item2}");
                            Console.WriteLine($"Doors: {string.Join(", ", doors[room])}");
                            Console.WriteLine($"Items: {string.Join(", ", items[room])}");
                        }

                        Console.WriteLine($"All: {string.Join(", ", items.Values.SelectMany(i => i).OrderBy(i => i))}");

                        Console.WriteLine($"Security is at {securityLoc.Item1}, {securityLoc.Item2}");
...
drop tambourine".Split(new[] { '\r' }, StringSplitOptions.RemoveEmptyEntries);//.Concat(null).ToArray();
                ...
                //    if (directions.TryGetValue(input, out var d))
                //    {
                //        x += d.Item1;
                //        y += d.Item2;
                //    }
...
".Replace("\r", ""))
        ...
        private static string[] magicPaths = new []
        {
            @"south
south
south
...
west",
        ...
        private class MapNode : Node<MapNode, string, int>
        {
            ...
            public override IEnumerable<MapNode> GetAdjacent()
            {
                ...
                if (wasSecurity)
                {
                    Console.WriteLine($"Testing security: {this.Description}");
                }

                var hasDoors = dirRes.Where(i => i.Value.IsMatch(output)).Select(i => i.Key).ToArray();
                //Console.WriteLine("Has doors: " + string.Join(", ", hasDoors));
                foreach (var door in hasDoors.Concat(_doors))
                {
                    var x = _x;
                    var y = _y;
                    var d = directions[door];
                ...
                if (itemsHere.Success)
                {
                    var thisItems = itemsRe.Matches(itemsHere.Groups[1].Value).Select(i => i.Groups[1].Value).ToArray();
            ...
            {
                get { return _moves.Length > 100; }
            }

            public override int CurrentCost
            {
                get { return _moves.Length; }
            }
            ...
            {
                get { return $"{_x},{_y} + {string.Join(",", _items)} = {CurrentCost} -> {string.Join(",", _moves)}"; }
            }
        }
