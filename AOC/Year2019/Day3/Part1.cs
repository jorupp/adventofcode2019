...
namespace AoC.Year2019.Day3
{
    public class Part1 : BasePart
    {
        protected void RunScenario(string title, string input)
        {
            RunScenario(title, () =>
            {
                ...
                var q = (from l1 in wireLines[0].Skip(1)
                    ...
                    .Select(i => i.Value)
                    .ToList();
                Console.WriteLine(string.Join(" / ", q.Select(i => $"{i.X},{i.Y}")));
                var d = q.Min(i => Math.Abs(i.X) + Math.Abs(i.Y));

                Console.WriteLine(d);
            });
        }

        private IEnumerable<(Point, Point)> Translate(List<string> paths)
        {
            var lastPoint = new Point(0, 0);
            foreach (var move in paths)
            {
                var nextPoint = Move(lastPoint, move);
                yield return (lastPoint, nextPoint);
                lastPoint = nextPoint;
            }
        }

        private Point? Intersection((Point, Point) l1, (Point, Point) l2)
        {
            ...
            if (h1)
            {
                ...
                else
                {
                    ...
                    {
                        return new Point(l2.Item1.X, l1.Item1.Y);
                    }
        ...
        private bool IsHorizontal((Point, Point) l)
        {
        ...
        private Point Move(Point start, string path)
        {
