    ...
    public class Part2 : BasePart
    {
        protected void RunScenario(string title, string input)
        {
            RunScenario(title, () =>
            {
                ...
                var wires = lines.Select(i => i.Split(',').ToList()).ToList();

                var wireLines = wires.Select(i => Translate(i).ToList()).ToList();

                var q = (from l1 in wireLines[0].Skip(1)
                         from l2 in wireLines[1].Skip(1)
                         select Intersection(l1, l2)
                         )
                         .Where(i => i.HasValue)
                         .Select(i => i.Value)
                    .ToList();
        ...
        private IEnumerable<(Point, Point, int)> Translate(List<string> paths)
        {
            var lastPoint = new Point(0, 0);
            var lastTraveled = 0;
            foreach (var move in paths)
            {
                var (nextPoint, traveled) = Move(lastPoint, move);
                yield return (lastPoint, nextPoint, lastTraveled);
                lastTraveled += traveled;
                lastPoint = nextPoint;
            }
        }

        private int? Intersection((Point, Point, int) l1, (Point, Point, int) l2)
        {
            var h1 = IsHorizontal(l1);
            var h2 = IsHorizontal(l2);
            if (h1)
            {
                if (h2)
                {
                    // both H
                    if (l1.Item1.Y != l2.Item1.Y)
                    {
                        return null;
                    }
                    throw new NotImplementedException("H same Y");
                }
                else
                {
                    // l1 is H, l2 is V
                    if (IsBetween(l1.Item1.X, l2.Item1.X, l1.Item2.X) && IsBetween(l2.Item1.Y, l1.Item1.Y, l2.Item2.Y))
                    {
                        //Console.WriteLine($"{l1.Item1.X},{l1.Item1.Y} -> {l1.Item2.X},{l1.Item2.Y}");
                        return l1.Item3 + Math.Abs(l1.Item1.X - l2.Item1.X) + l2.Item3 + Math.Abs(l2.Item1.Y - l1.Item1.Y);
                    }
            ...
            else
            {
                if (h2)
                {
                    // l1 is V, l2 is H - handle the inverse
                    return Intersection(l2, l1);
                }
                else
                {
                    // both V
                    if (l1.Item1.X != l2.Item1.X)
                    {
                        return null;
                    }
                    throw new NotImplementedException("V same X");
                }

            }
        }

        private bool IsHorizontal((Point, Point, int) l)
        {
            return l.Item1.Y == l.Item2.Y;
        }

        private bool IsBetween(int e1, int v, int e2)
        {
            return Math.Min(e1, e2) <= v && v <= Math.Max(e1, e2);
        }

        private (Point, int) Move(Point start, string path)
        {
            var d = int.Parse(path.Substring(1));
            switch (path[0])
            {
                case 'R':
                    return (new Point(start.X + d, start.Y), d);
                case 'U':
                    return (new Point(start.X, start.Y + d), d);
                case 'D':
                    return (new Point(start.X, start.Y - d), d);
                case 'L':
                    return (new Point(start.X - d, start.Y), d);
