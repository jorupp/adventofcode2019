    ...
    public class Part1 : BasePart
    {
        ...
        protected void RunScenario(string title, string input)
        {
            ...
            RunScenario(title, () =>
            {
                ...
                while (true)
                {
                    var newData = data.Select(i => i.ToList()).ToList();
                    var changed = false;
                    for (var x = 0; x < data.Count; x++)
                    {
                        for (var y = 0; y < data[0].Count; y++)
                        {
                            ...
                            foreach (var d in directions)
                            {
                                ...
                                if (nx < 0 || data.Count <= nx)
                                {
                                    //Console.WriteLine($"Skipped x = {nx}");
                                    continue;
                                }

                                if (ny < 0 || data[0].Count <= ny)
                                {
                                    //Console.WriteLine($"Skipped y = {ny}");
                                    continue;
                                }

                                //Console.WriteLine(data[nx][ny]);
                                if (data[nx][ny] == occupied)
                                {
                                    count++;
                                }
                            }
                            ...
                            if (data[x][y] == empty && count == 0)
                            {
                                changed = true;
                                newData[x][y] = occupied;
                            }
                            else if (data[x][y] == occupied && count >= 4)
                            {
                                changed = true;
                                newData[x][y] = empty;
                            }
                    ...
                    //for (var x = 0; x < data.Count; x++)
                    //{
                    //    Console.WriteLine(new string(data[x].ToArray()));
                    //}
                ...
                Console.WriteLine(data.Sum(i => i.Count(ii => ii == occupied)));
            });
        }

        public override void Run()
        {
            RunScenario("initial", @"L.LL.LL.LL
LLLLLLL.LL
L.L.L..L..
LLLL.LL.LL
L.LL.LL.LL
...
L.LLLLL.LL");
            //return;
            //return;
            RunScenario("part1", @"LLLLLL.LL.LLLLLL.LLLLLLL.LLLLLL.LLLLL.LLLLLLLLLLLLLLLLLL.LLLL.L.LLL.LLLLLLLL.LLLLLLLL.LLLLL
