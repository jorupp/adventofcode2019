...
namespace AoC.Year2020.Day17
{
    public class Part1 : BasePart
    {
        ...
        private readonly (int, int, int)[] directions;

        private void Dump(Dictionary<(int, int, int), bool> input)
        {
            ...
            for (var z = mz; z <= xz; z++)
            {
                for (var y = my; y <= xy; y++)
                {
                    for (var x = mx; x <= xx; x++)
                    {
                        Console.Write( (input.TryGetValue((x,y,z), out var v) && v) ? active : inactive);
                    }
                    Console.WriteLine();
                }
        ...
        private Dictionary<(int, int, int), bool> Simulate(Dictionary<(int, int, int), bool> input)
        {
            var output = new Dictionary<(int, int, int), bool>();
            ...
            for (var x = mx; x <= xx; x++)
            {
                for (var y = my; y <= xy; y++)
                {
                    for (var z = mz; z <= xz; z++)
                    {
                        var state = input.TryGetValue((x, y, z), out var v1) && v1;
                        var count = 0;
                        foreach (var dir in directions)
                        {
                            ...
                            var y1 = y + dir.Item2;
                            var z1 = z + dir.Item3;
                            if (input.TryGetValue((x1, y1, z1), out var v) && v)
                            {
                                count++;
                            }
                        }
