...
namespace AoC.Year2020.Day19
{
    public class Part2 : BasePart
    {
        ...
        private class Rule8 : Rule
        {
            public Rule8(Dictionary<string, Rule> rules):  base(rules)
            {
            }
            ...
            public override string GetPattern()
            {
                ...
                return "(" + Rules["42"].GetPattern() + ")+";
            }
        }

        private class Rule11 : Rule
        {
            private int Iterations;
            public Rule11(Dictionary<string, Rule> rules, int iterations) : base(rules)
            {
            ...
            public override string GetPattern()
            {
                ...
                var p42 = Rules["42"].GetPattern();
                var p31 = Rules["31"].GetPattern();

                string generate(int i)
                {
                    return "(" + string.Join("",
                               Enumerable.Range(0, i).Select(i => p42)
                                   .Concat(Enumerable.Range(0, i).Select(i => p31))) + ")";
                }

                return "(" + string.Join("|", Enumerable.Range(1, Iterations - 1).Select(i => generate(i))) + ")";
            }
        }
        ...
        {
            RunScenario("initial", @"42: 9 14 | 10 1
9: 14 27 | 1 26
10: 23 14 | 28 1
1: ""a""
11: 42 31
