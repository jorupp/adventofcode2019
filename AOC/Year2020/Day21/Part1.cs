    ...
    public class Part1 : BasePart
    {
        protected void RunScenario(string title, string input)
        {
            RunScenario(title, () =>
            {
                ...
                var reContains = new Regex(@"^(.*) \(contains (.*)\)$");

                var iCount = new Dictionary<string, long>();
                var aToI = new Dictionary<string, List<string>>();

                foreach (var line in lines)
                {
                    var m = reContains.Match(line);
                    var ingredients = line;
                    var alergens = "";
                    if (m.Success)
                    {
                        ingredients = m.Groups[1].Value;
                        alergens = m.Groups[2].Value;
                    }

                    var allI = ingredients.Split(' ');
                    var allA = alergens.Replace(",","").Split(' ');

                    foreach (var a in allA)
                    {
                        if (aToI.TryGetValue(a, out var ai))
                        {
                            aToI[a] = ai.Intersect(allI).ToList();
                        }
                        else
                        {
                            aToI[a] = allI.ToList();
                    ...
                    foreach (var i in allI)
                    {
                        var c = iCount.TryGetValue(i, out var ii) ? ii : 0;
                        iCount[i] = c + 1;
                    }
                }
                ...
                while (true)
                {
                    Console.WriteLine("Remaining: " + aToI.Count(i => i.Value.Count > 1));
                    var thisTime = aToI.Count(i => i.Value.Count > 1);
                    if (thisTime == last)
                    {
                        break;
                    }
                    ...
                    foreach (var a in aToI.Where(i => i.Value.Count == 1))
                    {
                        var doneA = a.Value.Single();
                        foreach (var k in aToI.Keys)
                        {
                            if (k == a.Key)
                            {
                                continue;
                ...
                var hasA = aToI.Values.SelectMany(i => i).ToHashSet();

                var appears = iCount.Where(i => !hasA.Contains(i.Key)).Sum(i => i.Value);

                Console.WriteLine(appears);


                //Arrange the ingredients alphabetically by their allergen and separate them by commas to
                //produce your canonical dangerous ingredient list. (There should not be any spaces in your
                //canonical dangerous ingredient list.)
                //In the above example, this would be mxmxvkd,sqjhc,fvjkl.


                Console.WriteLine(string.Join(",", aToI.OrderBy(i => i.Key).Select(i => i.Value.Single())));
            });
        }

        private void Dump(Dictionary<string, List<string>> aToI)
        {

            foreach (var kvp in aToI)
            {
