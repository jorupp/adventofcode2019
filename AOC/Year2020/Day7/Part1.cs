        ...
        protected void RunScenario(string title, string input)
        {
            RunScenario(title, () =>
            {
                ...
                foreach (var line in lines)
                {
                    ...
                    {
                        var parts = allInside.Split(',').Select(i => i.Trim()).ToArray();
                        foreach (var part in parts)
                        {
                            var bag = part.Trim("1234567890 ".ToArray());
                            bag = bag.Replace(" bags", "").Replace(" bag","");
                            if (!reverse.ContainsKey(bag))
                            {
                                reverse[bag] = new List<string>();
                            }

                            reverse[bag].Add(outside);
                        }
                    }
                }

                var toCheck = new Stack<string>();
                toCheck.Push("shiny gold");
                var done = new HashSet<string>();
                do
                {
                    var check = toCheck.Pop();
                    if (reverse.ContainsKey(check))
                    {
                        Console.WriteLine($"finding bags to put {check} in");
                        foreach (var inner in reverse[check])
                        {
                            if (!done.Contains(inner))
                            {
                                Console.WriteLine($"  Will check {inner}");
                                toCheck.Push(inner);
                                done.Add(inner);
                            }
                            else
                            {
                                Console.WriteLine($"  Already checked {inner}");
                    ...
                    {
                        Console.WriteLine($"{check} doesn't go in any bags");
                    }
