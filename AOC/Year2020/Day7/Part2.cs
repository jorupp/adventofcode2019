    ...
    public class Part2 : BasePart
    {
        protected void RunScenario(string title, string input)
        {
            RunScenario(title, () =>
            {
                ...
                var forward = new Dictionary<string, List<(string, int)>>();
                var re = new Regex(@"(.*) bags contain (.*)\.");
                var reBag = new Regex(@"(\d+) ([^,]*) bags?");
                foreach (var line in lines)
                {
                    var match = re.Match(line);
                    var outside = match.Groups[1].Value;
                    forward.Add(outside, new List<(string, int)>());
                    var allInside = match.Groups[2].Value;
                    if (allInside != "no other bags")
                    {
                        var matches = reBag.Matches(allInside);
                        foreach (var m in matches.Cast<Match>())
                        {
                            forward[outside].Add((m.Groups[2].Value, int.Parse(m.Groups[1].Value)));
                        }
                    }
                }
                ...
                int countInner(string bag)
                {
                    if (cache.TryGetValue(bag, out var v))
                    {
                        Console.WriteLine($"Cache hit: {bag}");
                        return v;
                    }
                    Console.WriteLine($"Cache miss: {bag}");

                    v = 0;
                    foreach (var t in forward[bag])
                    {
                        v += countInner(t.Item1) * t.Item2 + t.Item2;
                    }
                ...
                Console.WriteLine(countInner("shiny gold"));
            });
        }

        public override void Run()
        {
            RunScenario("initial", @"light red bags contain 1 bright white bag, 2 muted yellow bags.
...
dotted black bags contain no other bags.");
            ...
            RunScenario("initial-2", @"shiny gold bags contain 2 dark red bags.
...
dark violet bags contain no other bags.");
                //return;
                RunScenario("part1", @"muted white bags contain 4 dark orange bags, 3 bright white bags.
