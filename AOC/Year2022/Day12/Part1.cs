...
namespace AOC.Year2022.Day12
{
    ...
    {
        // 9:32:12 - 10:07:18, time: 35m06s, 100 @ 7m39s
        // I forgot that my A* algo needs to *include* the cost to the current node in the value of EstimatedCost....
        // 346 was too high
        // 320 (guess) was too low, 340 (guess) was too high
        protected void RunScenario(string title, string input)
        {
            RunScenario(title, () =>
            {
                ...
                var result = s.Evaluate<MapNode, string, decimal>(new MapNode(lines, start, end, 0, null));
        ...
        private class MapNode : Node<MapNode>
        {
            ...
            public MapNode(char[][] map, (int x, int y) point, (int x, int y) target, int cost, MapNode prior)
            {
                this.map = map;
                this.point = point;
                this.target = target;
            ...
            public override bool IsComplete => point.x == target.x && point.y == target.y;

            public override decimal CurrentCost => cost;

            public override decimal EstimatedCost => CurrentCost + Math.Abs(point.x - target.x) + Math.Abs(point.y - target.y) + 1;
            ...
            public override IEnumerable<MapNode> GetAdjacent()
            {
                foreach(var dir in directions)
                {
                    ...
                    if (tH <= cH + 1)
                    {
                        ...
                        yield return new MapNode(map, p2, target, cost + 1, this);
                    }
                }
            }
        }
